...
namespace WsdlExMachina.CSharpGenerator.Tests
{
    public class RoslynClientGeneratorTests
    {
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void GenerateClient_ShouldThrowArgumentException_WhenNamespaceIsNullOrEmpty(string namespaceName)
        {
            // Arrange
            var wsdl = CreateMinimalWsdlDefinition();

            // Act & Assert
            Assert.Throws<ArgumentException>(() => _generator.GenerateClient(wsdl, namespaceName));
        }
