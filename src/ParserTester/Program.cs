...
class Program
{
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"Found {wsdl.PortTypes.Count} port types with {wsdl.PortTypes.Sum(pt => pt.Operations.Count)} operations");
            Console.WriteLine($"Found {wsdl.Types.ComplexTypes.Count} complex types");
            Console.WriteLine($"Found {wsdl.Types.SimpleTypes.Count} simple types");
            Console.WriteLine($"Found {wsdl.Types.Elements.Count} elements");

            // Generate C# code
            Console.WriteLine("Generating C# code...");

            // Extract namespace from WSDL target namespace
            string namespaceName = new TypeMapper().GetCSharpNamespace(wsdl.TargetNamespace);
