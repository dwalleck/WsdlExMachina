...
public abstract class SoapClientBase
{
    ...
    /// </summary>
    protected string EndpointUrl { get; set; } = string.Empty;
    ...
    protected async Task<string> SendSoapRequestAsync(string soapEnvelope, string soapAction)
    {
        ...
        var response = await _retryPolicy.ExecuteAsync(() => _httpClient.PostAsync(EndpointUrl, content)).ConfigureAwait(false);
    ...
    protected T DeserializeResponse<T>(string responseContent)
    {
        ...
        var responseNode = xmlDoc.SelectSingleNode("//soap:Body/*[1]", nsManager) ?? throw new InvalidOperationException("Response element not found in SOAP body.");
